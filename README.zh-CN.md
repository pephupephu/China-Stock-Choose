...
> 邮箱，同时把可读化的结果提交到这个仓库。**股票池仅限沪深京 A 股**，
> ST / \*ST / B 股 / 港股在数据抓取阶段直接剔除，不会出现在任何分支
> 也不会出现在邮件正文里。所有数据均来自公开披露来源（巨潮 / 新浪 /
> 同花顺 / 申万），不引入任何第三方研报、雪球评论、媒体观点或付费数据源。

| 项目 | 内容 |
| --- | --- |
| **仓库地址** | https://github.com/pephupephu/China-Stock-Choose |
| **运行环境** | Python 3.10+，AKShare（巨潮 / 新浪 / 同花顺 / 申万数据） |
| **股票池** | 沪深京 A 股（不含 ST / B 股 / 港股） |
...
- 邮件正文**只展示命中清单**（或"今日无命中"提示），被剔除的标的
  不会进邮件（避免旧经济周期 / ST 票干扰阅读），完整审计数据见
  `pick_YYYY-MM-DD.json`；
...
python -m src.main run --limit 200   # 快速本地测试（取前 200 只）
```
...
| 阶段 | 过滤 | 原因 |
| --- | --- | --- |
| Universe | 剔除 ST / \*ST 简称 | 始终不参与打分 |
| Universe | 剔除 200xxx（深圳 B 股）、9xxxxx（上海 B 股） | 不是 A 股 |
| Universe | 剔除 6 位数字外的代码（含港股 5 位代码等） | 不是 A 股 |
...
| 编号 | 规则 | 数据来源 |
| --- | --- | --- |
| 1 | 剔除 ST / \*ST 股票（在 Universe 阶段已完成） | 简称字符判定 |
| 2 | 剔除年报非标审计意见股票（默认 `RULE_EXCLUDE_QUALIFIED=true`） | 财报附注披露 |
| 3 | **近 3 年**股息率**每年均** ≥ 4%（按现金分红 / 期末收盘价计算，**不做每股分红金额过滤**；时间窗通过 `RULE_DIVIDEND_LOOKBACK_YEARS=3` 调节） | 巨潮 `stock_dividend_cninfo` |
...
| 11 | 近 3 年主营收入 YoY 最大跌幅 ≤ 20%（`metrics._annual_yoy_pct(window=3)`） | 新浪 / 同花顺 |
...
- 每股 OCF、负债率、分红支付率
- **近 3 年**每股分红、**近 3 年**股息率（年报对应收盘价）
- **近 3 年**主营收入 YoY（仅最近 3 年，不再列 1990 起的所有历史）
- **在建工程 / 重大事项**（取自同花顺主营构成 + 年报附注，便于人工复查）
- 一次性特别分红、审计意见、负债率/分红现金流等警示标签
- 评分 `score`（按综合评分降序，邮件正文只展示前 50 条）

**被规则剔除的标的不会出现在邮件正文**（避免 ST / 旧经济周期类被剔
标的干扰阅读；如需完整审计数据请打开 `pick_YYYY-MM-DD.json`）。
...
output/pick_YYYY-MM-DD.json    # 完整结构化结果（含被剔除的标的）
```
...
| 密钥 | 备注 |
| --- | --- |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USE_SSL` | 如 `smtp.163.com:465` |
...
- 本项目对 ST / \*ST、B 股、港股代码一律不纳入筛选，且不在邮件中
  出现，但仅依赖公开数据，仍可能漏算或滞后；以原始公告为准。
