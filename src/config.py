...
class ScreenerRules:
    ...
    min_dividend_yield_pct: float = 3.0  # ponytail: relaxed 4.0 -> 3.5 -> 3.0 per user (Sept 13/14)
    ...
    min_roe_pct: float = 6.0           # ponytail: relaxed 10.0 -> 8.0 -> 6.0 per user (Sept 13/14)
    roe_strict_all_years: bool = True
    max_revenue_decline_pct: float = 30.0  # ponytail: relaxed 20.0 -> 30.0 per user (Sept 14)   # YoY single-year max drop in main revenue
    revenue_decline_eval_years: int = 3

    # Capital structure
    max_debt_ratio_pct: float = 80.0  # ponytail: relaxed 70.0 -> 75.0 -> 80.0 per user (Sept 13/14)
    warn_debt_ratio_pct: float = 60.0
    require_positive_cf_per_share: bool = True

    # Distribution
    min_payout_ratio_pct: float = 20.0  # ponytail: relaxed 40.0 -> 25.0 -> 20.0 per user (Sept 13/14)
...
def load_config(env_path: Optional[Path] = None) -> AppConfig:
    ...
    rules = ScreenerRules(
        min_dividend_yield_pct=float(env.get("RULE_MIN_DIVIDEND_YIELD_PCT", 3.0)),
        dividend_lookback_years=int(env.get("RULE_DIVIDEND_LOOKBACK_YEARS", 3)),
        max_pe_ttm=float(env.get("RULE_MAX_PE_TTM", 35.0)),
        max_debt_ratio_pct=float(env.get("RULE_MAX_DEBT_RATIO_PCT", 80.0)),
        warn_debt_ratio_pct=float(env.get("RULE_WARN_DEBT_RATIO_PCT", 60.0)),
        max_revenue_decline_pct=float(env.get("RULE_MAX_REVENUE_DECLINE_PCT", 30.0)),
        min_payout_ratio_pct=float(env.get("RULE_MIN_PAYOUT_RATIO_PCT", 20.0)),
        min_roe_pct=float(env.get("RULE_MIN_ROE_PCT", 6.0)),
